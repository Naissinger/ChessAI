...
class Browser {
    ...
    async noLogin() {
        ...
        try {
            await this.driver.get('https://chess.com/play/online');
            ...
            const labelBeginner = await this.driver.executeScript(`return document.querySelector('label[for="advanced"]')`);
            const beginnerButton = await labelBeginner.findElement(By.className('authentication-intro-name'));
            await this.driver.executeScript(`arguments[0].click();`, beginnerButton);
    ...
    async turnHintsOn() {
        ...
        try {
            ...
            await this.driver.wait(until.elementLocated(By.name('theme.piece_style')), 0);

            await this.driver.executeScript(`document.querySelector('input[id="board.show_legal_moves"').scrollIntoView({'behavior': 'smooth'})`);

            const labelCheck = await this.driver.executeScript(`return document.querySelector('label[for="board.show_legal_moves"]');`);
            const labelCheckColor = await labelCheck.getCssValue("background-color");

            if (labelCheckColor == "rgba(119, 155, 77, 1)" || labelCheckColor == "#779b4d") {
                await this.driver.executeScript(`document.querySelector('button[data-cy="cancel"]').click();`);
            } else {
                await this.driver.executeScript(`document.querySelector('input[id="board.show_legal_moves"').click();`);
                await this.driver.executeScript(`document.querySelector('button[data-cy="save"]').click();`);
            }
        }
        catch (e) { console.log(e); }
    }
    ...
    async personalizeBoard(piece, board) {
        try {
            ...
            await this.driver.executeScript(`document.querySelector('select[name="theme.piece_style"]').scrollIntoView({'behavior': 'smooth'});`);
            await this.driver.findElement(By.name('theme.piece_style')).click();

            const pieceInputElement = await this.driver.executeScript(`return document.querySelector('select[name="theme.piece_style"');`);

            let options = await pieceInputElement.findElements(By.tagName('option'));

            for (let i = 0; i < options.length; i++) {
                if ((await options[i].getText()).trim() == piece) {
                    await options[i].click();
                    await this.driver.findElement(By.name('theme.piece_style')).click();
                }
            }

            await this.driver.executeScript(`document.querySelector('select[name="theme.board_style"]').scrollIntoView({'behavior': 'smooth'});`);
            await this.driver.findElement(By.name('theme.board_style')).click();

            const boardInputElement = await this.driver.executeScript(`return document.querySelector('select[name="theme.board_style"');`);

            options = await boardInputElement.findElements(By.tagName('option'));

            for (let i = 0; i < options.length; i++) {
                if ((await options[i].getText()).trim() == board) {
                    await options[i].click();
                    await this.driver.findElement(By.name('theme.board_style')).click();
                }
            }
    ...
    async inicia() {
        ...
        await this.personalizeBoard("Ocean", "Icy Sea");
